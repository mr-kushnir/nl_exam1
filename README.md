...
|----------|-------------|
| [CLAUDE.md](CLAUDE.md) | Technical documentation, architecture, development log |
| [DEVGUIDE.md](DEVGUIDE.md) | Development principles: zero manual code, agent pipeline |
...
Пишите или говорите боту **как угодно** — он поймёт. YaGPT распознаёт естественный язык.

```
кофе 300                                    # стандартно
маме перевёл тыщу                           # разговорно
жене 500 и маме 500 и на пиво 1000          # несколько сразу
```

Поддерживается **несколько расходов** в одном сообщении — бот разберёт и запишет каждый отдельно.

Также можно отправить **голосовое сообщение** — Yandex SpeechKit распознает речь.
...
├── DEVGUIDE.md              # Принципы разработки
...
Проект разрабатывается полностью автоматически через систему агентов:

```
Human (Epic) → BUSINESS → DEVELOPER → TESTER → SECURITY → DEPLOYER → Production
```

**Принципы:**
- Ни строчки кода руками - всё через агенты
- BDD сценарии хранятся в YouTrack Knowledge Base
- Задачи трекаются в YouTrack с привязкой к KB статьям
- TDD: RED → GREEN → REFACTOR для каждого сценария
- Автоматический security scan и deploy

Подробнее: [DEVGUIDE.md](DEVGUIDE.md)
