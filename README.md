...
Пишите или говорите боту **как угодно** — он поймёт. Используется YaGPT для распознавания естественного языка.

```
кофе 300                    # стандартно
маме перевёл тыщу           # разговорно
бенз на 2к                  # сленг
вчера mass effect за 3500   # с контекстом
```

Также можно отправить **голосовое сообщение** — бот распознает речь через Yandex SpeechKit.
